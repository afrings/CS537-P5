...
main(void){
    ...
    // Call the mencrypt with partially valid range
    if (mencrypt((char *)ptr, 2) != -1)
        err("mencrypt return -1 when [vaddr, vaddr + len * PGSIZE) resides in valid region\n");
    ...
    if (getpgtable(pt_entries, PAGES_NUM) >= 0){
        for (int i = 0; i < PAGES_NUM; i++) {
            ...
            if (is_failed) {
                ...
                err("None of the pages should be encrypted when -1 is returned\n");
            }
